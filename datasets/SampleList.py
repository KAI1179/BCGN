...
class SampleList(object):
    ...
    def __init__(self, img, image_size, mode="xyxy"):
    ...
    # def _copy_extra_fields(self, img):
    #     for k, v in img.extra_fields.items():
    ...
    def to(self, device):
        sample = SampleList(self.img.to(device), self.size, self.mode)
        for k, v in self.extra_fields.items():
            if hasattr(v, "to"):
                v = v.to(device)
            if k in self.triplet_extra_fields:
                sample.add_field(k, v, is_triplet=True)
            else:
                sample.add_field(k, v)
        return sample

    def __getitem__(self, item):
        bbox = SampleList(self.img[item], self.size, self.mode)
        for k, v in self.extra_fields.items():
            if k in self.triplet_extra_fields:
    ...
    def clip_to_image(self, remove_empty=True):
        TO_REMOVE = 1
        self.img[:, 0].clamp_(min=0, max=self.size[0] - TO_REMOVE)
        self.img[:, 1].clamp_(min=0, max=self.size[1] - TO_REMOVE)
        self.img[:, 2].clamp_(min=0, max=self.size[0] - TO_REMOVE)
        self.img[:, 3].clamp_(min=0, max=self.size[1] - TO_REMOVE)
        if remove_empty:
            box = self.img
    ...
    def copy(self):
        return SampleList(self.img, self.size, self.mode)

    def copy_with_fields(self, fields, skip_missing=False):
        bbox = SampleList(self.img, self.size, self.mode)
