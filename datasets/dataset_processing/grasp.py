...
class GraspRectangles:
    ...
    @classmethod
    def load_from_cornell_file_vmrd(cls, fname, scale_factor=(1, 1)):
        """
        Load grasp rectangles from a Cornell dataset grasp file.
        :param fname: Path to text file.
        :return: GraspRectangles()
        ...
        # all_grasps = {'grasps': grasp_mat, 'grasp_inds': ind}

        # perm = [2, 3, 0, 1, 6, 7, 4, 5] ## VMRD 适应 cornell  从左上角 顺时针
        # perm = [4, 5, 6, 7, 0, 1, 2, 3] ## VMRD 适应 cornell  从左下角 逆时针

        for i in range(grasp_mat.shape[0]):
            _tmp = grasp_mat[i]
            # tmp = np.array([_tmp[2], _tmp[3], _tmp[0], _tmp[1], _tmp[6], _tmp[7], _tmp[4], _tmp[5]]).reshape(4, 2)  ## 注意， 以下 这里 坐标点 应该表示为 (y1, x1), (y2, x2)...
            # tmp = np.array([_tmp[3], _tmp[2], _tmp[1], _tmp[0], _tmp[7], _tmp[6], _tmp[5], _tmp[4]]).reshape(4, 2)  ##  从左上角 顺时针
            tmp = np.array([_tmp[3], _tmp[2], _tmp[1], _tmp[0], _tmp[7], _tmp[6], _tmp[5], _tmp[4]]).reshape(4, 2)  ##  从左下角 逆时针
            # tmp = grasp_mat[i].reshape(4, 2)
            # tmp[:, 0] *= sca_w
            # tmp[:, 1] *= sca_h
            tmp[:, 1] *= sca_w
            tmp[:, 0] *= sca_h
            tmp = np.rint(tmp)
            grs.append(GraspRectangle(tmp))


        return cls(grs), ind

    @classmethod
    def load_from_cornell_file_vmrd_for_ours(cls, fname, scale_factor=(1, 1)):
        """
        Load grasp rectangles from a Cornell dataset grasp file.
        :param fname: Path to text file.
        :return: GraspRectangles()
        ...
        grses = []
        grses_ind = []
        grs_1, grs_2, grs_3, grs_4, grs_5, grs_6, grs_7 = [], [], [], [], [], [], []
        (sca_h, sca_w) = scale_factor
        ...
        for i in range(grasp_mat.shape[0]):
            _tmp = grasp_mat[i]
            tmp = np.array([_tmp[2], _tmp[3], _tmp[0], _tmp[1], _tmp[6], _tmp[7], _tmp[4], _tmp[5]]).reshape(4, 2)  ## 注意， 以下 这里 坐标点 应该表示为 (y1, x1), (y2, x2)...
            # tmp = np.array([_tmp[1], _tmp[0], _tmp[3], _tmp[2], _tmp[5], _tmp[4], _tmp[7], _tmp[6]]).reshape(4, 2)  ## 注意， 以下 这里 坐标点 应该表示为 (y1, x1), (y2, x2)...
            # tmp = np.array([[_tmp[1], _tmp[0]], [_tmp[3], _tmp[2]], [_tmp[5], _tmp[4]], [_tmp[7], _tmp[6]]])  ##
            # tmp = np.array([_tmp[3], _tmp[2], _tmp[1], _tmp[0], _tmp[7], _tmp[6], _tmp[5], _tmp[4]]).reshape(4, 2)  ##  从左下角 逆时针

            # tmp = np.array([_tmp[6], _tmp[7], _tmp[0], _tmp[1], _tmp[2], _tmp[2], _tmp[4], _tmp[5]]).reshape(4, 2)  ## 注意， 以下 这里 坐标点 应该表示为 (y1, x1), (y2, x2)...
            ...
            tmp = np.rint(tmp)
            if ind[i] == 1:
                grs_1.append(GraspRectangle(tmp))
            elif ind[i] == 2:
                grs_2.append(GraspRectangle(tmp))
            elif ind[i] == 3:
                grs_3.append(GraspRectangle(tmp))
            elif ind[i] == 4:
                grs_4.append(GraspRectangle(tmp))
            elif ind[i] == 5:
                grs_5.append(GraspRectangle(tmp))
            elif ind[i] == 6:
                grs_6.append(GraspRectangle(tmp))
            elif ind[i] == 7:
                grs_7.append(GraspRectangle(tmp))
            else:
                assert "wrong!!!"
        if 1 in ind:
            grses.append(cls(grs_1))
            grses_ind.append(1)
        if 2 in ind:
            grses.append(cls(grs_2))
            grses_ind.append(2)
        if 3 in ind:
            grses.append(cls(grs_3))
            grses_ind.append(3)
        if 4 in ind:
            grses.append(cls(grs_4))
            grses_ind.append(4)
        if 5 in ind:
            grses.append(cls(grs_5))
            grses_ind.append(5)
        if 6 in ind:
            grses.append(cls(grs_6))
            grses_ind.append(6)
        if 7 in ind:
            grses.append(cls(grs_7))
            grses_ind.append(7)


        # return cls(grs), ind
        return grses, grses_ind
    ...
    def draw(self, shape, ind, node_ids, position=True, angle=True, width=True):  ## 改动 for VMRD
        """
        ...
        obj_num = node_ids.max()
        (w, h) = shape
        shape_vmrd = (obj_num, w, h)
        ind_int = ind.astype(int)

        # if obj_num != ind_int.max():
        #     print('00000000')

        if position:
            pos_out = np.zeros(shape_vmrd)
        else:
            pos_out = None
        if angle:
            ang_out = np.zeros(shape_vmrd)
        else:
            ang_out = None
        if width:
            width_out = np.zeros(shape_vmrd)
        else:
            width_out = None
        ...
        for gr in self.grs:
            ...
            if position:
                pos_out[(ind_int[gr_index]-1), rr, cc] = 1.0
            if angle:
                ang_out[(ind_int[gr_index]-1), rr, cc] = gr.angle
            if width:
                width_out[(ind_int[gr_index]-1), rr, cc] = gr.length
            gr_index += 1

        return pos_out, ang_out, width_out

    def draw_for_baseline(self, shape, position=True, angle=True, width=True):  ## 多物体抓取
        """
        Plot all GraspRectangles as solid rectangles in a numpy array, e.g. as network training data.
        :param shape: output shape
        :param position: If True, Q output will be produced
    ...
    def draw_for_ours(self, shape, position=True, angle=True, width=True):  ## 多物体抓取
        """
        Plot all GraspRectangles as solid rectangles in a numpy array, e.g. as network training data.
        :param shape: output shape
        :param position: If True, Q output will be produced
...
class GraspRectangle:
    ...
    def compact_polygon_coords(self, shape=None):
        ...
        # return Grasp(self.center, self.angle, self.length / 5, self.width).as_gr.polygon_coords(shape)
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    # local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.04, num_peaks=no_grasps)
