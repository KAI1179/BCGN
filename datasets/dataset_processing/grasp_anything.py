...
class GraspRectangles:
    ...
    @classmethod
    def load_from_vmrd_file_gai(cls, fname, obj_index):
        """
        Load grasp rectangles from a VMRD dataset grasp file.
        :param fname: Path to text file.
        :return: GraspRectangles()
        ...
        with open(fname) as f:
            ...
            for grasp_line in grasp_lines:
                # x1, y1, x2, y2, x3, y3, x4, y4 = list(map(lambda x: int(round(float(x))), grasp_line.split(' ')[:8]))
                y1, x1, y2, x2, y3, x3, y4, x4 = list(map(lambda x: int(round(float(x))), grasp_line.split(' ')[:8]))
                ind = int(grasp_line.split(' ')[8])

                if ind == int(obj_index):
                    try:
                        gr = np.array([
                            [x1, y1],
                            [x2, y2],
        ...
        # @classmethod
        # def load_from_vmrd_file_gai_linshi(cls, fname, obj_index):
        #     """
        #     Load grasp rectangles from a VMRD dataset grasp file.
        #     :param fname: Path to text file.
        #     :return: GraspRectangles()
        ...
        #
        #                 if ind == 1:
        #                     grs_1.append(GraspRectangle(gr))
        #                 elif ind == 2:
        #                     grs_2.append(GraspRectangle(gr))
        #                 elif ind == 3:
        #                     grs_3.append(GraspRectangle(gr))
        #                 elif ind == 4:
        #                     grs_4.append(GraspRectangle(gr))
        #                 elif ind == 5:
        #                     grs_5.append(GraspRectangle(gr))
        #                 elif ind == 6:
        #                     grs_6.append(GraspRectangle(gr))
        #                 elif ind == 7:
        #                     grs_7.append(GraspRectangle(gr))
        #
        #             except ValueError:
        #             # Some files contain weird values.
        #             continue
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    # local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.05, num_peaks=no_grasps)
    # local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.3, num_peaks=no_grasps)
    # local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.3, num_peaks=5)


    # import pandas as pd
    # df = pd.DataFrame(q_img)
    # df.to_excel('q_img.xlsx', index=False)
